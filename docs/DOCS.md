...
| R54 | **Speech recognition fires "no match" too quickly on desktop** | With `continuous: false`, Chrome fires `onend` immediately on silence (~500ms), jumping to "missed" before the user can speak. Restarting the same instance fails on Chrome. Fix: switched all audio pages to `recognition.continuous = true` so recognition stays open and keeps listening until a match is found or the hard timeout (8-10s) fires. `onerror` ignores "no-speech"/"aborted" events (normal in continuous mode). `onend` only marks missed when triggered by explicit `.stop()`. Applied to: Word Echo (`app/intake/communication/page.tsx`), Speech Practice (`app/kid-dashboard/speech/page.tsx`), Audio Intake (`app/intake/audio/page.tsx`). |
