...
| R51 | **Desktop mic not working in Word Echo and Speech Practice** | SpeechRecognition.start() was called only 200ms after TTS audio ended — not enough on Windows/Chrome for hardware release. Fix: increased delay to 500ms post-TTS + added 300ms explicit wait after speakWord resolves before entering listening state. Added `recognition.onend` handler for silence timeout. Added `maxAlternatives: 3` and checks all alternatives for matches. Explicitly releases audio element (`audio.src = ""`) after TTS ends. `app/intake/communication/page.tsx`, `app/kid-dashboard/speech/page.tsx` |
...
| R53 | **No audio visualizer during listening** | Listening state only showed a pulsing dot. Fix: added real-time mic visualizer using `getUserMedia` + `AudioContext` + `AnalyserNode` — renders 5 reactive bars on a canvas that respond to actual microphone input levels. Added CSS fallback visualizer (animated bars) for TTS playing state. Red pulsing recording indicator during listening. Speech practice page also gets animated bar visualizer in the "Listening..." button. `app/intake/communication/page.tsx`, `app/kid-dashboard/speech/page.tsx` |
